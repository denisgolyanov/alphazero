...
class MCTSSimulator(object):
    def __init__(self, network, initial_state):
        ...
        """
        self._root = RootNode(network, initial_state)
    ...
    def _compute_action_probabilities(self, temperature):
        def _child_node_probability(child_node):
            return child_node.visit_count ** temperature / \
                (sum([child_node.visit_count ** temperature for child_node in self._root.children]))

        return [(child_node.action, _child_node_probability(child_node)) for child_node in self._root.children]

    def _compute_next_action_competitive(self):
        best_child_node = max(self._root.children, key=lambda child_node: child_node.visit_count)
        logging.info('Children: %s', str([str(child_node) for child_node in self._root.children]))
        logging.info('Scores: %s', str([str(child_node.selection_score) for child_node in self._root.children]))

        return best_child_node.action

    def _compute_next_action_stochastic(self):

        total_visit_count = sum([child_node.visit_count for child_node in self._root.children])

        def _child_node_probability(child_node):
            return child_node.visit_count / (1.0 * total_visit_count)

        probabilities = [_child_node_probability(child_node) for child_node in self._root.children]
        logging.info('Move probabilities: %s', str(probabilities))
        logging.info('Children: %s', str([str(child_node) for child_node in self._root.children]))
        logging.info('Scores: %s', str([str(child_node.selection_score) for child_node in self._root.children]))

        return np.random.choice(self._root.children, p=probabilities).action

    def compute_next_action(self, num_simulations=1600):
        for i in range(num_simulations):
            logging.debug("Simulation %d", i)
